...
export interface TableSetupHardwareTabActions {
    canSave: boolean;
    isSaving: boolean;
    save: () => void;
}
...
interface TableSetupHardwareTabProps {
    ...
    isActive: boolean;
    onClose: () => void;
    onActionsChange: (actions: TableSetupHardwareTabActions | null) => void;
}

export function TableSetupHardwareTab({
    tableId,
    isOpen,
    isActive,
    onClose,
    onActionsChange,
}: TableSetupHardwareTabProps) {
    const enabled = isOpen && isActive;

    const utils = api.useUtils();
    const query = api.setup.get.useQuery({ tableId }, { enabled });
    const saveMut = api.setup.save.useMutation({
        onSuccess: async () => {
            await utils.setup.get.invalidate({ tableId });
            onClose();
        },
    });
    ...
    const save = React.useCallback(() => {
        ...
        saveMut.mutate({
            tableId,
            dealerSerial: form.dealerSerial,
            dealerPublicKey: form.dealerPublicKey,
            scannerSerial: form.scannerSerial,
            scannerPublicKey: form.scannerPublicKey,
            handSerials: form.handSerials,
            handPublicKeys: form.handPublicKeys,
        });
    }, [enabled, query.data, saveMut, tableId]);
    ...
    }, [canSave, isActive, onActionsChange, save, saveMut.isPending]);
